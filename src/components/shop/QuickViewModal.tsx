      ...
      {product && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="relative w-full max-w-5xl bg-[#0a0a0a] border border-white/10 rounded-[3rem] overflow-hidden shadow-[0_0_100px_rgba(0,0,0,0.8)] overflow-y-auto max-h-[90vh]"
          >
            <button 
              onClick={onClose}
              className="absolute top-8 right-8 z-50 p-3 bg-white/5 hover:bg-white/10 text-white rounded-full transition-all border border-white/10"
            >
              <X size={20} />
            </button>

            <div className="grid grid-cols-1 md:grid-cols-2 min-h-[600px]">
              <div className="relative bg-white/5 flex flex-col">
                ...
                {allMedia.length > 1 && (
                  <div className="flex gap-2 p-4 overflow-x-auto no-scrollbar justify-center border-t border-white/5">
              ...
              <div className="p-8 md:p-16 flex flex-col bg-gradient-to-br from-[#0a0a0a] to-[#111]">
                <div className="mb-10">
                  <div className="flex items-center gap-3 mb-6">
                    <span className="px-4 py-1.5 bg-blue-600/20 border border-blue-500/30 text-blue-400 rounded-full text-[10px] font-black uppercase tracking-[0.2em]">
                      {product.category}
                    </span>
                    <span className={cn(
                      "px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-[0.2em] border",
                      (product.stock || 0) > 0 ? "bg-emerald-500/20 text-emerald-400 border-emerald-500/30" : "bg-red-500/20 text-red-500 border-red-500/30"
                    )}>
                      {(product.stock || 0) > 0 ? `${product.stock} Units Syncing` : 'Depleted'}
                    </span>
                  </div>
                  <h2 className="text-5xl font-black text-white tracking-tighter mb-4 uppercase italic leading-[0.9]">
                    {product.name}
                  </h2>
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs font-black text-gray-500 uppercase tracking-widest font-mono">
                ...
                <div className="mb-12">
                  <div className="text-[10px] text-gray-500 font-black uppercase tracking-widest mb-2 italic">Standard Retail Value</div>
                  <p className="text-5xl font-black text-white font-mono tracking-tighter italic">UGX {product.price.toLocaleString()}</p>
                </div>
                ...
                <div className="space-y-8 mb-12 flex-1">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-5 bg-white/5 rounded-3xl border border-white/5 hover:border-white/10 transition-colors">
                       <h4 className="text-[9px] font-black text-gray-600 uppercase tracking-widest mb-2 flex items-center gap-2"><Zap size={14} className="text-yellow-500" /> Integrity</h4>
                       <p className="text-white font-black text-xs uppercase italic tracking-tighter">Factory Fresh</p>
                    </div>
                    <div className="p-5 bg-white/5 rounded-3xl border border-white/5 hover:border-white/10 transition-colors">
                       <h4 className="text-[9px] font-black text-gray-600 uppercase tracking-widest mb-2 flex items-center gap-2"><Truck size={14} className="text-blue-500" /> Logistics</h4>
                       <p className="text-white font-black text-xs uppercase italic tracking-tighter">Ready for Dispatch</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <button 
                    ...
                    className="group py-6 bg-white hover:bg-blue-500 text-black hover:text-white font-black rounded-2xl flex items-center justify-center gap-3 transition-all active:scale-95 text-[11px] uppercase tracking-[0.2em] shadow-xl disabled:opacity-50"
                  >
                    <ShoppingCart size={18} className="group-hover:scale-110 transition-transform" /> Commit to Inventory
                  </button>
                  <button onClick={handleWhatsAppBuy} className="py-6 bg-green-600/10 hover:bg-green-600/20 text-green-500 border border-green-500/20 font-black rounded-2xl flex items-center justify-center gap-3 transition-all active:scale-95 text-[11px] uppercase tracking-[0.2em]">
